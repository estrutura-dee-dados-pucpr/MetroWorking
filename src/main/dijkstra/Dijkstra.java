...
public class Dijkstra {
	...
	public static int pesoDaRota(ArrayList<Rota> rota) {
		...
		for (int i = 1; i < rota.size(); i++) {
			if(!rota.get(i - 1).getCor().equals(rota.get(i).getCor())){
				System.out.println("SOMEI");
				res += 6;
			}
			res += rota.get(i).getPeso();
		}
		res += rota.get(0).getPeso();
		return res;
	...
	public Rota[][] initMatriz(Rota[] rotas) {
		...
		for (Rota r : rotas) {
			...
			res[r.getChegada() - 1][r.getSaida() - 1] = r;
	...
	public void mostraResposta(ArrayList<Rota> rotas, StringBuilder res, int i) {
		...
		if (!rotas.isEmpty()) {
			...
			res.append(String.format("Pegar linha %s de %d a %d (%d)\n",
					tmp1.getCor(), tmp1.getSaida(), tmp1.getChegada(),
					tmp1.getPeso()));
			mostraResposta(rotas, res, i++);
		}
