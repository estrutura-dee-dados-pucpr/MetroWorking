...
public class Gui extends JFrame implements MouseListener {
	...
	public void mouseClicked(MouseEvent e) {
		...
		if (saida != -1 && chegada != -1 && saida != chegada) {
			calculaDij();
	...
	class CircleList {
		...
		public void drawRes(String res, Graphics g){
			if(res== null)
				return;
			System.out.println(res);
			String[] a = res.split("\n");
			for(int i =0; i < a.length; i++){
				((Graphics2D)g).clearRect(1000, 10 + i*50,1000, 1000);
				System.out.println("Painting null");
			}
			for(int i =0; i < a.length; i++){
				((Graphics2D)g).drawString(a[i], 1000 , 100+ i*50);
				System.out.println("Painting" + a[i]);
			}
		}
